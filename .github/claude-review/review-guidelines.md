...
Where a rule is stated both here and in `AGENTS.md`, `AGENTS.md` wins: the bullet here is the stale
one, and the disagreement is itself a finding to raise. That overlap is deliberate rather than an
oversight — a rule reads there as an instruction to whoever writes the code, and here as the check
that produces a finding, carrying the severity calibration and the exemptions only a reviewer needs.

Two bodies of rules sit outside that arrangement, in opposite directions. The UI house style is kept
out of this file entirely, because it is long, purely prescriptive, and would drift the fastest;
section 6 carries only its grading. Most of section 4's screening checks run the other way and are
review-side only, since they describe what to look for in a contribution rather than what an author
should write. The exceptions are its dependency-vetting and licensing checks, which `AGENTS.md` does
state and which the precedence rule above therefore covers; for the rest, no rule there backs them
and none is stale for lacking one.
...
- Layer boundaries: a new import in `src/libs/` reaching up into framework code (`src/composables/`, `src/stores/`, a component, or `vue`) breaks the rule in the AGENTS.md "Separation of concerns" section, which carries the startup mechanism. Grade it by what the import does at load time, not by its path. It is `major` when a store getter (`useSomeStore()`) is called at the module scope of the importing lib — check that first, since it needs no second file opened and is where the call usually sits — or of the imported module, or of an app-internal (`@/`) module it imports, or when the new edge closes an import cycle; that read stops at external packages, which cannot close an edge back into `src/`. The same call inside a function body runs after bootstrap, so that is the line to draw. A module-scope `getActivePinia()` or `inject()` returns `undefined` instead of throwing, so the app boots and then misbehaves: grade that as the dead-value bug it is, not as a startup hazard. A type-only import is erased at compile time, and an inert declaration (`ref()`, `computed()`, `reactive({})`, literals) touches no app instance; both are the rule's boundary, so report them at most as `minor` layering drift and never as a startup hazard. A module-scope `watch()` is not one of them — in a `.ts` module no component scope owns it and nothing stops it, so it runs for the life of the process and is a cleanup finding on its own merits, at whatever severity that earns. Existing violations elsewhere in the tree do not license a new one.
...
- Migration logic belongs in `src/utils/migrations.ts`, never inside a Pinia store.
...
- House style: Cockpit's UI conventions are defined in the `UI house style` section of `AGENTS.md`, which is the single source for them — judge added UI against that section rather than against a copy kept here, and cite the rule you are applying. Not all of it describes the current tree: unless you can check a clause against in-tree precedent for what it asks, treat one that wants something added or changed (an accessible name, a token, a relocated action) as the house direction the codebase is still converging on. That framing lowers the consistency grade, not the severity: a breach that still reaches the user (an overlay rendering light and unreadable, a control no keyboard can reach) stays `major` whether or not the clause is house direction, while one that is only cosmetically inconsistent with the app around it (an action nobody can tell is the primary one) is `minor`. When a single surface breaches several of them, group the breaches under one `6.x` finding with one sub-item per fix, so each fix stays individually actionable without inflating the finding count the verdict is read against.
- Grading those rules. The section states what the code should do; these are the calibrations and exemptions that decide whether a breach is a finding, and at what severity:
  - Dialog anatomy: judge the anatomy, not the component providing it — a hand-rolled dialog satisfies it by gaining the close X, never by being told to migrate to a different shell.
  - `theme="dark"`: 26 of the 32 `v-select` in the tree already carry it, so this one is in-tree precedent rather than house direction, and a diff that removes it is a regression to raise rather than a cleanup to wave through.
  - Button tokens: the opaque `color="white"` fill still common on footer commits is the legacy form being replaced, graded `minor` — not the same mistake as `color="primary"`. In a hand-rolled footer, or one written into the `useInteractionDialog` `#actions` slot where the author styles each button, also flag a committing action left indistinguishable from the dismiss. A footer passed through the shell's `actions` prop is exempt: the prop styles its buttons uniformly and the primary is differentiated by passing a fill class (`bg-[#FFFFFF33]`) on the committing action.
  - Footer actions: the few multi-action footers still in the tree are the form being replaced, so grade an added third action as `minor`, and a lone action on the left the same. A footer passed through the shell's `actions` prop exposes no per-action variant, so do not demand `variant="text"` on the dismiss there.
  - Icon-only controls: `minor` when the glyph or an adjacent label already conveys the action, `major` only when the unlabeled control is the user's sole affordance, so a screen reader announces nothing.
...
- Separation of concerns: flag non-trivial business/domain logic (calculations, parsing, transformations, protocol handling, validation) implemented inline in a `.vue` `<script>` that should live in a framework-agnostic `.ts` module under `src/libs/` (no `vue`, composable, store or component imports). Trivial glue may stay in the component.
