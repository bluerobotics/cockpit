      ...
      <v-container>
        <div class="flex flex-col h-[70vh] border border-[rgba(255,255,255,0.3)] rounded elevation-3">
            ...
            <table class="w-full border-collapse">
              <thead>
                <tr class="bg-[#00000022]">
                  ...
                  <th class="text-center w-[485px] p-2 border-b border-b-[rgba(255,255,255,0.3)]">Value</th>
                </tr>
              </thead>
            </table>
          </div>
                      ...
                      <template v-if="isPrimitive(editedValues[item.originalKey])">
                        <div class="text-center py-0 cursor-pointer" @dblclick="startEditing(item)">
                          <template v-if="editing[item.originalKey]">
                            <template v-if="typeof editedValues[item.originalKey] === 'boolean'">
                              <select
                                v-model="editedValues[item.originalKey]"
                                class="bg-[#FFFFFF11] w-[120px]"
                                @change="() => commitChanges(item)"
                              >
                            ...
                            <template v-else-if="typeof editedValues[item.originalKey] === 'number'">
                              <input
                                ...
                                type="number"
                                class="text-right bg-transparent w-[150px]"
                                @blur="() => commitChanges(item)"
                              />
                            </template>
                          ...
                          <template v-else>
                            <span>{{ editedValues[item.originalKey] }}</span>
                          </template>
                        </div>
                      </template>
                      <template v-else>
          ...
          >
            ...
            <v-tooltip text="Upload and aplly config file">
            ...
            <v-tooltip text="Save config file">
              <template #activator="{ props: tooltipProps }">
...
const userSettings = useStorage<Settings>('settings', {})
...
// Updates the local edited values as userSettings changes
watch(
  userSettings,
  (newVal) => {
    Object.assign(editedValues, newVal)
  },
  { deep: true }
)
...
const getUserSettingsFromBlueOS = async (): Promise<Record<string, any>> => {
  const vehicleAddress = vehicleStore.globalAddress
  const username = missionStore.username
  const settings = await getKeyDataFromCockpitVehicleStorage(vehicleAddress, `settings/${username}`)
...
const loadUserSettings = async (): Promise<void> => {
  if (vehicleStore.isVehicleOnline) {
    try {
      const settings = await getUserSettingsFromBlueOS()
      userSettings.value = settings
      Object.assign(editedValues, settings)
    } catch (error: any) {
      openSnackbar({
        message: 'Failed to load settings from user profile. ' + error.message,
        variant: 'error',
        duration: 5000,
      })
    }
