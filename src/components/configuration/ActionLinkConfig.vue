<template>
  <v-dialog v-model="dialog.show" max-width="540px">
    <v-card class="rounded-lg" :style="interfaceStore.globalGlassMenuStyles">
      <v-card-title class="text-h6 font-weight-bold py-4 text-center">{{ $t('Automatic Triggers') }}</v-card-title>
      <v-card-subtitle class="text-caption text-center"
        >{{ $t('Action') }}: {{ dialog.action?.name ?? $t('Unknown') }}</v-card-subtitle
      >
      <v-card-text class="px-8">
        <p class="text-subtitle-1 font-weight-bold mb-2">
          {{ $t('When variable(s) change:') }}
        </p>

        <v-autocomplete
          v-model="dialog.selectedVariables"
          :items="availableDataLakeVariables"
          :label="$t('Data Lake Variables')"
          multiple
          chips
          variant="outlined"
          density="compact"
        ...
        <div class="mt-4">
          ...
          <v-radio-group v-model="dialog.autoRunType" inline class="mt-2">
            <v-radio class="mx-2" value="none" :label="$t('No time trigger')"></v-radio>
            <v-radio class="mx-2" value="once" :label="$t('On startup')"></v-radio>
            <v-radio class="mx-2" value="interval" :label="$t('Repeatedly')"></v-radio>
          </v-radio-group>
          <div v-if="dialog.autoRunType !== 'none'" class="mt-4">
            <v-slider
              ...
              :label="dialog.autoRunType === 'interval' ? $t('Run every') : $t('Run after')"
              thumb-label
            >
              <template #append>
                <v-text-field
                  ...
                  style="width: 140px"
                  variant="outlined"
                  :suffix="runDelaySeconds == 1 ? $t('second') : $t('seconds')"
                  hide-details
                  theme="dark"
                />
              </template>
      ...
      <v-card-actions>
        <div class="flex justify-between items-center pa-2 w-full h-full">
          ...
          <v-btn :disabled="!isFormValid" @click="saveConfig">{{ $t('Save') }}</v-btn>
        </div>
      </v-card-actions>
    </v-card>
  </v-dialog>
...
const saveConfig = (): void => {
  if (!dialog.value.action) {
    openSnackbar({ message: t('Action is required'), variant: 'error' })
    return
  }

  if (dialog.value.minInterval <= 0) {
    openSnackbar({ message: t('Variable-based frequency must be greater than 0'), variant: 'error' })
    return
  }

  if (dialog.value.autoRunType === 'interval' && dialog.value.delayMs < 1) {
    openSnackbar({ message: t('Interval-based delay must be at least 1 millisecond'), variant: 'error' })
    return
  }

  if (dialog.value.autoRunType === 'once' && dialog.value.delayMs < 0) {
    openSnackbar({ message: t('Startup delay cannot be negative'), variant: 'error' })
    return
  }
