  ...
  <v-dialog v-model="dialog.show" max-width="540px">
    <v-card class="rounded-lg" :style="interfaceStore.globalGlassMenuStyles">
      <v-card-title class="text-h6 font-weight-bold py-4 text-center">{{
        $t('configuration.actions.actionLink.automaticTriggers')
      }}</v-card-title>
      <v-card-subtitle class="text-caption text-center"
        >{{ $t('configuration.actions.actionLink.action') }}:
        {{ dialog.action?.name ?? $t('configuration.actions.actionLink.unknown') }}</v-card-subtitle
      >
      <v-card-text class="px-8">
        <p class="text-subtitle-1 font-weight-bold mb-2">
          {{ $t('configuration.actions.actionLink.whenVariablesChange') }}
        </p>

        <v-autocomplete
          v-model="dialog.selectedVariables"
          :items="availableDataLakeVariables"
          :label="$t('configuration.actions.actionLink.dataLakeVariables')"
          multiple
          chips
          variant="outlined"
          density="compact"
        ...
        <v-text-field
          v-model="frequencyHz"
          :label="$t('configuration.actions.actionLink.noFasterThan')"
          :suffix="$t('configuration.actions.actionLink.changesPerSecond')"
          theme="dark"
          type="number"
          min="0"
          variant="outlined"
        ...
        <div class="mt-4">
          <p class="text-subtitle-1 font-weight-bold mb-2">{{ $t('configuration.actions.actionLink.timeBased') }}</p>
          <v-radio-group v-model="dialog.autoRunType" inline class="mt-2">
            <v-radio class="mx-2" value="none" :label="$t('configuration.actions.actionLink.noTimeTrigger')"></v-radio>
            <v-radio class="mx-2" value="once" :label="$t('configuration.actions.actionLink.onStartup')"></v-radio>
            <v-radio class="mx-2" value="interval" :label="$t('configuration.actions.actionLink.repeatedly')"></v-radio>
          </v-radio-group>
            ...
            <v-slider
              ...
              :label="
                dialog.autoRunType === 'interval'
                  ? $t('configuration.actions.actionLink.runEvery')
                  : $t('configuration.actions.actionLink.runAfter')
              "
              thumb-label
            >
              <template #append>
                <v-text-field
                  ...
                  :suffix="
                    runDelaySeconds == 1
                      ? $t('configuration.actions.actionLink.second')
                      : $t('configuration.actions.actionLink.seconds')
                  "
                  hide-details
                  theme="dark"
                />
...
const saveConfig = (): void => {
  if (!dialog.value.action) {
    openSnackbar({ message: t('configuration.actions.actionLink.actionRequired'), variant: 'error' })
    return
  }

  if (dialog.value.minInterval <= 0) {
    openSnackbar({ message: t('configuration.actions.actionLink.frequencyError'), variant: 'error' })
    return
  }

  if (dialog.value.autoRunType === 'interval' && dialog.value.delayMs < 1) {
    openSnackbar({ message: t('configuration.actions.actionLink.intervalError'), variant: 'error' })
    return
  }

  if (dialog.value.autoRunType === 'once' && dialog.value.delayMs < 0) {
    openSnackbar({ message: t('configuration.actions.actionLink.startupDelayError'), variant: 'error' })
    return
  }
