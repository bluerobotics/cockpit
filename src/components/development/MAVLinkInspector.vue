<template>
  <div class="flex flex-col gap-4">
    <div class="flex flex-row gap-4 mb-4 flex-wrap">
      <div>
        <div class="text-lg mb-2">{{ t('Available message types') }}</div>
        <div class="mb-2 flex items-center">
          ...
          />
          <v-btn variant="outlined" class="rounded-md ml-2" @click="resetTrackedMessageTypes">{{ t('Reset') }}</v-btn>
        </div>
      ...
      <div v-if="trackedMessageTypes.size > 0" class="w-auto mr-2">
        <div class="text-lg mb-2">{{ t('Message values') }}</div>
