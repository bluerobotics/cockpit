  ...
  >
    <v-card class="pa-4 text-white rounded-lg" :style="interfaceStore.globalGlassMenuStyles">
      <v-card-title class="flex justify-between items-center">
        <span class="text-lg font-medium">BlueOS Cloud login</span>
      ...
      <v-card-actions class="px-4 py-3">
        <v-spacer />
        <v-btn v-if="step === 'success'" variant="text" @click="closeDialog">Done</v-btn>
        <template v-else>
...
const displayName = computed(() => {
  const u = cloudStore.user
  if (!u) return ''
  return u.name || u.nickname || u.email || u.sub
})
