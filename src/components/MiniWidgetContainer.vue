...
const widgetAdded = (e: SortableEvent.SortableEvent): void => {
  ...
  if (newWidget && e.pullMode === 'clone') {
    newWidget.hash = uuid()
    logUserAction(`Added mini-widget '${newWidget.component}' to '${container.value.name}'`)
...
const onDragEnd = (): void => {
  logUserAction(`Ended dragging mini-widget from container '${container.value.name}'`)
