      ...
      <v-card-text class="px-8">
        <div class="flex flex-col gap-4">
          ...
          <v-checkbox
            ...
            label="Persist Between Boots"
            hint="If checked, this variable will be saved and restored when Cockpit restarts"
            persistent-hint
            hide-details
          />
          <v-checkbox
...
import { createDataLakeVariable, DataLakeVariable, updateDataLakeVariableInfo } from '@/libs/actions/data-lake'
...
const editMode = computed(() => !!props.editVariable)
...
watch(
  ...
  (newValue) => {
    if (newValue) {
      variable.id = newValue.id
      variable.name = newValue.name
...
const saveVariable = (): void => {
  ...
  if (editMode.value) {
    ...
    createDataLakeVariable(variable, parsedValue)
  }
