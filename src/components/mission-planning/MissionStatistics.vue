<template>
  ...
  >
    <p class="text-sm font-semibold mb-[6px]">Mission statistics</p>
    ...
    <div class="text-xs leading-6">
      ...
      <div class="flex justify-between">
        <span>ETA</span><span>{{ timeToCompleteMission }}</span>
      </div>
      <div class="flex justify-between">
        <span>Energy</span><span>{{ totalEnergy }}</span>
      </div>
...
defineProps<{
  /**
   * Whether the mission statistics panel is visible
   */
  modelValue: boolean
}>()
...
const { totalMissionLength, timeToCompleteMission, totalEnergy, totalSurveyCoverage } = useMissionStats()
