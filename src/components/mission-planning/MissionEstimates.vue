<template>
    ...
    <v-card :style="interfaceStore.globalGlassMenuStyles">
      ...
      <v-card-actions>
        <div class="flex justify-between w-full pa-1">
          ...
          <v-btn color="white" @click="isSettingsOpen = false">{{ $t('common.close') }}</v-btn>
        </div>
      </v-card-actions>
    </v-card>
  </v-dialog>
  <v-dialog v-model="isAboutMessageOpen" persistent max-width="600px">
    <v-card :style="interfaceStore.globalGlassMenuStyles">
      <v-card-title class="text-lg text-center font-semibold">{{ $t('missionEstimates.aboutEstimates') }}</v-card-title>
      ...
      <v-card-actions>
        <div class="flex justify-end w-full pa-1">
          <v-btn color="white" @click="isAboutMessageOpen = false">{{ $t('common.close') }}</v-btn>
        </div>
      </v-card-actions>
    </v-card>
  </v-dialog>
