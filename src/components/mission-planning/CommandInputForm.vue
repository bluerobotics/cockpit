  ...
  <div class="command-form">
    <div class="flex flex-col px-3 my-2 bg-[#EEEEEE22] text-white rounded-md">
      ...
      <div class="flex flex-col w-full justify-between items-center text-[12px] border-b border-white/10 py-2">
        <p class="w-full text-center">{{ t('Command type') }}</p>
        <v-select
      ...
      <div v-if="selectedCommandType" class="flex flex-col w-full justify-between items-center text-[12px] py-2">
        <p class="w-full text-center">{{ t('Command') }}</p>
        <v-select
      ...
      <div v-if="selectedMavCommand" class="flex flex-col">
        ...
        <div class="text-[11px] font-semibold text-center py-2">{{ t('Parameters') }}</div>
        ...
        <template v-if="selectedCommandType === MissionCommandType.MAVLINK_NAV_COMMAND">
          ...
          >
            <p class="w-[80px] text-start opacity-75">{{ t('Param') }} {{ paramNum }}:</p>
      ...
      <div class="flex w-full justify-between py-2">
        <v-btn size="small" variant="outlined" @click="cancelCommand"> {{ t('Cancel') }} </v-btn>
...
const commandTypeOptions = [
  { name: t('MAVLink Navigation Command'), value: MissionCommandType.MAVLINK_NAV_COMMAND },
  { name: t('MAVLink Non-Navigation Command'), value: MissionCommandType.MAVLINK_NON_NAV_COMMAND },
]
