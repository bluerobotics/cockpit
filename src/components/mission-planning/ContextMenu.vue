          ...
          <template #activator="{ props: tooltipProps1 }">
            <v-btn
              ...
              :style="[
                interfaceStore.globalGlassMenuStyles,
                {
                  borderColor: isCreatingSimpleMission
                    ? '#FFFF0022'
                    : interfaceStore.globalGlassMenuStyles.backgroundColor,
                  backgroundColor: '#333333EE',
                },
              ]"
              variant="elevated"
...
const emit = defineEmits<{
  ...
  (event: 'undoGenerateWaypoints'): void
...
const surveyCreationButtonText = computed(() => {
  ...
  if (missionStore.surveys.length === 0) {
