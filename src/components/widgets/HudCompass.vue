...
const angleX = (angle: number): number => {
  const diff = angle - degrees(store.attitude.yaw || 0)
  let x = -diff
  if (-diff < -180) {
    x = 360 - diff
  } else if (-diff > 180) {
  ...
  return (widget.value.options.yawGainFactor * radians(x)) / Math.cos(radians(x))
}
...
const renderCanvas = (): void => {
  ...
  for (const [angle, x] of Object.entries(renderVars.yawLinesX)) {
    ctx.beginPath()
    ctx.moveTo(halfCanvasWidth + x, refFontSize + stdPad + refTriangleSize + stdPad)
    ctx.lineTo(halfCanvasWidth + x, canvasSize.value.height - linesFontSize - stdPad - minorLinesGap)
    ctx.lineWidth = '1'

    // For angles that are multiple of 15 degrees, use a bolder line and write angle down
    if (Number(angle) % 15 === 0) {
      ctx.lineWidth = '2'
      ctx.lineTo(halfCanvasWidth + x, canvasSize.value.height - linesFontSize - stdPad)
      ctx.fillText(angleRender(Number(angle)), halfCanvasWidth + x, canvasSize.value.height - stdPad)
    }
    ctx.stroke()
  }
...
watch(yaw, () => {
  const anglesPositions: { [angle: string]: number } = {}
  yawAngles.forEach((angle: number) => {
    anglesPositions[angle] = angleX(angle)
  })

  Object.entries(anglesPositions).forEach(([angle, position]) => {
    // Only interpolate angle render with GSAP when the angle is not changing
    // sides, so it doesn't cross across the screen.
    if (Math.abs(renderVars.yawLinesX[angle] - position) > canvasSize.value.width / 4) {
