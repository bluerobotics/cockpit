  ...
  <div class="w-full h-full">
    ...
    <v-dialog v-model="widgetStore.widgetManagerVars(widget.hash).configMenuOpen" min-width="400" max-width="35%">
      <v-card class="pa-2" :style="interfaceStore.globalGlassMenuStyles">
        <v-card-title class="text-center">{{ $t('Video Source') }}</v-card-title>
        <v-card-text>
          <a>{{ $t('Video Source') }}</a>
          <v-text-field
            variant="filled"
          ...
          ></v-text-field>
          <div>
            <span class="text-xs font-semibold leading-3 text-slate-600">{{ $t('Fit style') }}</span>
            <Dropdown
              ...
              class="max-w-[144px]"
            />
            <v-checkbox v-model="widget.options.autoplay" :label="$t('Autoplay')" hide-details />
            <v-checkbox v-model="widget.options.controls" :label="$t('Show controls')" hide-details />
            <v-checkbox v-model="widget.options.loop" :label="$t('Loop')" hide-details />
            <v-checkbox v-model="widget.options.muted" :label="$t('Muted')" hide-details />
          </div>
        </v-card-text>
