<template>
  ...
  >
    <div :id="mapId" ref="map" class="map">
      ...
      <v-tooltip location="top" :text="$t('map.switchToMissionPlanning')">
...
const attachOfflineProgress = (layer: any, layerName: string): void => {
  layer.on('savestart', (e: any) => {
    ...
    openSnackbar({ message: t('info.savingTiles', { count: tilesTotal.value, layerName }), variant: 'info', duration: 2000 })
  })
...
const menuItems = computed(() => [
  ...
  {
    item: t('map.setGlobalOrigin'),
    action: () => onMenuOptionSelect('set-global-origin'),
    icon: 'mdi-crosshairs-question',
  },
  {
    item: t('map.placePointOfInterest'),
    action: () => onMenuOptionSelect('place-poi'),
    icon: 'mdi-map-marker-plus',
  },
  { item: t('map.goto'), action: () => onMenuOptionSelect('goto'), icon: 'mdi-crosshairs-gps' },
  {
    item: t('map.setDefaultMapPosition'),
...
const vehicleDownloadMissionButtonTooltipText = computed(() => {
  return vehicleStore.isVehicleOnline ? t('map.downloadMissionFromVehicleTooltip') : t('map.cannotDownloadMission')
})

const vehicleExecuteMissionButtonTooltipText = computed(() => {
  return vehicleStore.isVehicleOnline ? t('map.executeMissionFromVehicleTooltip') : t('map.cannotExecuteMission')
})
