      ...
      >
        ...
        <template #content>
          <div class="flex flex-col pt-1 w-full gap-y-1">
            ...
            <div class="config-row">
              <div class="config-label-with-info">
                <p class="config-label">Antenna height ({{ heightUnit }})</p>
              ...
              <input
                ...
                type="number"
                step="0.5"
                :min="heightToDisplayBound(ANTENNA_HEIGHT_LIMITS_M.min)"
                :max="heightToDisplayBound(ANTENNA_HEIGHT_LIMITS_M.max)"
                class="config-input"
        ...
        <template #content>
          <div class="flex flex-col pt-1 w-full gap-y-1">
            ...
            <div class="config-row config-checkbox-row">
              ...
              <v-checkbox
                :model-value="config.showSignalOnMap"
                hide-details
                density="compact"
                class="config-checkbox"
                @update:model-value="store.toggleSignalVisibility()"
              />
            </div>
...
const { toDisplayBound: heightToDisplayBound, unit: heightUnit } = useUnitConversion('m', 'altitude')
...
const onAntennaHeightBlur = (): void => {
  ...
  logUserAction(`Set the base station antenna height to ${heightToDisplayBound(height, 1)} ${heightUnit.value}`)
}
...
const saveCustomTileUrl = (): void => {
  logUserAction('Saved the base station custom coverage tile URL')
...
const onCoverageProviderChange = (event: Event): void => {
  ...
  logUserAction(`Switched the base station mobile coverage provider to "${next}"`)
...
const reloadMobileCoverage = (): void => {
  logUserAction('Reloaded the base station mobile coverage data')
  store.requestMobileCoverageReload()
}

const resetVisibleMobileCoverageData = (): void => {
  logUserAction('Reset the visible base station mobile coverage data')
  store.requestVisibleMobileCoverageDataReset()
}

const toggleOpenCellIdApiKeyField = (): void => {
  logUserAction(`${openCellIdApiKeyFieldOpen.value ? 'Hid' : 'Showed'} the OpenCellID API key field`)
...
const toggleMobileCoverageTargetTool = (): void => {
  logUserAction(
    `${store.mobileCoverageTargetToolActive ? 'Deactivated' : 'Activated'} the base station coverage target tool`
  )
...
onMounted(() => window.addEventListener('keydown', onTargetToolEscape))
onBeforeUnmount(() => {
  window.removeEventListener('keydown', onTargetToolEscape)
  store.mobileCoverageTargetToolActive = false
})
