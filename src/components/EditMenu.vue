              ...
              >
                ...
                >
                  ...
                  <p class="ml-3 overflow-hidden 2xl:text-sm text-xs text-ellipsis whitespace-nowrap">
                    {{ translateWidgetName(widget.name, widget.component) }}
                  </p>
                  <div class="grow" />
...
const translateWidgetName = (name: string, component?: string): string => {
  const lookup = component ?? name
  const nameKey = `components.widgets.${lookup}.name`
  const nameTranslated = t(nameKey)
  if (nameTranslated !== nameKey) return nameTranslated
  const stringKey = `components.widgets.${lookup}`
  const stringTranslated = t(stringKey)
  if (stringTranslated !== stringKey && typeof stringTranslated === 'string') return stringTranslated
  if (component && component !== name) {
    const fallbackKey = `components.widgets.${name}.name`
...
const translateMiniWidgetName = (name: string): string => {
  const nameKey = `components.mini-widgets.${name}.name`
  const nameTranslated = t(nameKey)
  if (nameTranslated !== nameKey) return nameTranslated
  const stringKey = `components.mini-widgets.${name}`
  const stringTranslated = t(stringKey)
  if (stringTranslated !== stringKey && typeof stringTranslated === 'string') {
...
const resetSavedProfiles = (): void => {
  showDialog({
    message: t('components.EditMenu.confirmResetProfiles'),
    actions: [
      {
      ...
      {
        text: t('components.EditMenu.resetProfiles'),
        action: () => {
...
const widgetModeOptions = computed(() => [
  { title: t('components.EditMenu.regular'), value: 'Regular' },
  { title: t('components.EditMenu.mini'), value: 'Mini' },
  { title: t('components.EditMenu.input'), value: 'Input' },
])
