        ...
        <div class="flex text-start 2xl:w-[260px] xl:w-[220px] w-[170px]">
          ...
          >
            <span
              ...
              {{ store.currentProfile.name.endsWith('profile') ? '' : t('editPanel.profile') }}
            </span>
          </v-btn>
        </div>
        <div
          ...
          >
            <div class="flex">
              ...
              <span
                ...
                {{ profile.name.endsWith('profile') ? '' : t('editPanel.profile') }}
              </span>
            </div>
          </div>
        </div>
        ...
        <v-menu offset-y theme="dark">
          ...
          <v-list>
            <div class="flex justify-center max-w-[250px] px-2 gap-x-[5px] pb-2">
              <p class="whitespace-nowrap">{{ $t('editPanel.settingsFor') }}</p>
            ...
            <v-list-item class="hover:bg-white/[0.04]">
              <label class="flex w-full h-full cursor-pointer justify-between">
                <v-list-item-title>{{ $t('editPanel.import') }}</v-list-item-title>
            ...
            <v-list-item @click="store.exportProfile(store.currentProfile)">
              <div class="flex w-full justify-between">
                <v-list-item-title>{{ $t('editPanel.export') }}</v-list-item-title>
                <v-icon size="20">mdi-download</v-icon>
              </div>
            </v-list-item>
            <v-list-item @click="store.duplicateProfile(store.currentProfile)">
              <div class="flex w-full justify-between">
                <v-list-item-title>{{ $t('editPanel.duplicate') }}</v-list-item-title>
                <v-icon size="20">mdi-content-copy</v-icon>
              </div>
            </v-list-item>
            <v-list-item @click="renameProfile(store.currentProfile)">
              <div class="flex w-full justify-between">
                <v-list-item-title>{{ $t('editPanel.configRename') }}</v-list-item-title>
            ...
            <div class="flex justify-center max-w-[250px] px-2 gap-x-[5px] pb-2 pt-1">
              <p class="whitespace-nowrap">{{ $t('editPanel.generalProfileSettings') }}</p>
            </div>
            <v-divider class="mb-1" />
            <v-list-item @click="addNewProfile">
              <div class="flex w-full justify-between mt-[6px]">
                <v-list-item-title>{{ $t('editPanel.addNewProfile') }}</v-list-item-title>
            ...
            <v-list-item @click="store.snapToGrid = !store.snapToGrid">
              <div class="flex w-full justify-between mt-[6px]">
                <v-list-item-title>{{
                  store.snapToGrid ? $t('editPanel.disableGrid') : $t('editPanel.enableGrid')
                }}</v-list-item-title>
            ...
            <v-list-item @click="resetSavedProfiles">
              <div class="flex w-full justify-between mt-[6px]">
                <v-list-item-title class="mr-6">{{ $t('editPanel.resetProfiles') }}</v-list-item-title>
...
const translateViewName = (viewName: string): string => {
  const nameMap: Record<string, string> = {
    'Video View': t('views["Video View"]'),
    'Map View': t('views["Map View"]'),
    'HUD View': t('views["HUD View"]'),
    'Map view': t('views["Map View"]'), // Handle lowercase variant
  }
...
const translateProfileName = (profileName: string): string => {
  const nameMap: Record<string, string> = {
    'ROV default': t('editPanel.rovDefault'),
    'Boat default': t('editPanel.boatDefault'),
    'MAV default': t('editPanel.mavDefault'),
  }
