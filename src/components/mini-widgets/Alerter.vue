<template>
        ...
        <div v-for="(alert, i) in sortedAlertsReversed" :key="alert.time_created.toISOString()">
          ...
          >
            ...
            >
              ...
              <p>{{ t(`alerts.levels.${alert.level}`) }}</p>
            </div>
          </div>
  ...
  >
    <template #content>
      <div class="flex items-start justify-between flex-col -mt-8 mb-6 pl-6">
        <v-switch
          ...
          hide-details
          :label="$t('alerter.enableColorCodedAlerts')"
          color="white"
        />
        <v-switch v-model="alertStore.enableVoiceAlerts" hide-details :label="$t('alerter.enableVoiceAlerts')" color="white" />
        <v-slider
    ...
    <template #actions>
      <v-btn @click="widgetStore.miniWidgetManagerVars(miniWidget.hash).configMenuOpen = false">{{ $t('common.close') }}</v-btn>
    </template>
  </InteractionDialog>
</template>
...
const { t } = useI18n()
const noRecentAlertsMessage = computed(() => t('alerts.noRecentAlerts'))
