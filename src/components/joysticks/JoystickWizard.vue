                    ...
                    <template v-if="step.inputType === 'axis'">
                      ...
                      >
                        ...
                        >
                          ...
                          <div
                            v-if="step.id === 3 && detectedAxisId !== null"
                            class="flex flex-col items-center justify-center w-[60px]"
                          >
                            <v-icon
                              class="text-2xl mb-1 transition-colors"
                              :class="heaveDirection === 'up' ? 'text-[#4faedd]' : 'text-[#ffffff55]'"
                              icon="mdi-arrow-up-bold"
                            />
                            <v-icon
                              class="text-2xl mt-1 transition-colors"
                              :class="heaveDirection === 'down' ? 'text-[#4faedd]' : 'text-[#ffffff55]'"
                              icon="mdi-arrow-down-bold"
                            />
                          </div>
                          <div
                            v-if="step.id === 4 && detectedAxisId !== null"
                            class="flex flex-row items-center justify-center w-[80px]"
                          >
                            <v-icon
                              class="text-2xl mr-2 transition-colors"
                              :class="swayDirection === 'left' ? 'text-[#4faedd]' : 'text-[#ffffff55]'"
                              icon="mdi-arrow-left-bold"
                            />
                            <v-icon
                              class="text-2xl ml-2 transition-colors"
                              :class="swayDirection === 'right' ? 'text-[#4faedd]' : 'text-[#ffffff55]'"
                              icon="mdi-arrow-right-bold"
                            />
                          </div>
                          <div
                            v-if="step.id === 5 && detectedAxisId !== null"
                            class="flex flex-col items-center justify-center w-[80px]"
                            style="transform: scaleY(0.3); transform-origin: center"
                          >
                            <v-icon
                              class="text-[44px] mb-1 transition-colors"
                              :class="forwardDirection === 'forward' ? 'text-[#4faedd]' : 'text-[#ffffff55]'"
                              icon="mdi-arrow-up-bold"
                            />
                            <v-icon
                              class="text-[52px] mt-1 transition-colors"
                              :class="forwardDirection === 'backward' ? 'text-[#4faedd]' : 'text-[#ffffff55]'"
                              icon="mdi-arrow-down-bold"
                            />
                          </div>
                          <div
                            v-if="step.id === 6 && detectedAxisId !== null"
                            class="flex flex-row items-center justify-center w-[80px]"
                          >
                            <v-icon
                              class="text-2xl mr-2 transition-colors"
                              :class="yawDirection === 'left' ? 'text-[#4faedd]' : 'text-[#ffffff55]'"
                              icon="mdi-arrow-u-down-left-bold"
                            />
                            <v-icon
                              class="text-2xl ml-2 transition-colors"
                              :class="yawDirection === 'right' ? 'text-[#4faedd]' : 'text-[#ffffff55]'"
...
const heaveDirection = computed<'up' | 'down' | 'neutral'>(() => {
  if (currentStepMeta.value?.id !== 3) return 'neutral'
  const val = currentAxisValue.value

  if (val < -axisThreshold) return 'up'
  if (val > axisThreshold) return 'down'
  return 'neutral'
})

const swayDirection = computed<'left' | 'right' | 'neutral'>(() => {
  if (currentStepMeta.value?.id !== 4) return 'neutral'
  const val = currentAxisValue.value

  if (val < -axisThreshold) return 'left'
  if (val > axisThreshold) return 'right'
  return 'neutral'
})

const forwardDirection = computed<'forward' | 'backward' | 'neutral'>(() => {
  if (currentStepMeta.value?.id !== 5) return 'neutral'

  const val = currentAxisValue.value

  if (val < -axisThreshold) return 'forward'
  if (val > axisThreshold) return 'backward'
  return 'neutral'
})

const yawDirection = computed<'left' | 'right' | 'neutral'>(() => {
  if (currentStepMeta.value?.id !== 6) return 'neutral'
