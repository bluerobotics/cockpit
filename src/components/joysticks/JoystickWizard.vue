...
const detectInputs = (state: JoystickState): void => {
  ...
  state.buttons.forEach((isDown, idx) => {
    // Falling edge detection for the start button (otherwise HOTAS controllers will trigger multiple inputs)
    ...
    // Keep button edge memory for next frame
  ...
  if (stepMeta?.inputType === 'axis') {
    ...
    state.buttons.forEach((isDown, idx) => {
      ...
      // Checks if the button is not already bound on this step, not the SHIFT key, and not a long pressed or held button.
      const canDetect =
        isDown && detectedButtonId.value === null && idx !== shiftButtonIndex.value && !heldButtons.has(idx)

      if (canDetect) {
        // If the button is the START button, we set a flag to prevent auto-advancing
        if (idx === startButtonIndex.value) {
          justMappedStartButton = true
        }
        // Binds the SHIFT button state at detection time (not live), so mapping is deterministic.
