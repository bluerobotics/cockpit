...
const boundAxisInfo = computed(() => {
  ...
  const idx = Object.keys(wizardJoystickMapping.value.axesCorrespondencies).find(
    (k) => wizardJoystickMapping.value.axesCorrespondencies[Number(k)].action.id === target.id
  )
...
const mapStepInput = (stepIdFinished: number): boolean => {
  ...
  if (axisConfigMapROV[stepIdFinished] && detectedAxisId.value !== null) {
    ...
    const { id, name, protocol } = axisConfigMapROV[stepIdFinished]
  ...
  if (buttonConfigMapROV[stepIdFinished] && detectedButtonId.value !== null) {
    ...
    // Binds the Shift button
    if (stepIdFinished === 7) {
      shiftButtonIndex.value = idx
      joystickWizardSteps.value[6].disableNextButton = false
      wizardJoystickMapping.value.buttonsCorrespondencies.regular[idx].action = buttonConfigMapROV[7]
      detectedButtonId.value = null
      return true
    }

    const { id, name, protocol } = buttonConfigMapROV[stepIdFinished]
    ...
    const isSameAction = !isNoFunction(existing) && existing.id === id
    ...
    wizardJoystickMapping.value.buttonsCorrespondencies[buttonType][idx].action = {
      id,
      name,
      protocol,
    }
...
const finishAndSaveProfile = async (): Promise<void> => {
  ...
  try {
    ...
    openSnackbar({
      message: 'Joystick profile saved successfully!',
      variant: 'success',
    })
    setTimeout(() => {
    ...
    openSnackbar({
      message: `Error saving joystick profile (${error}). Please try again.`,
      variant: 'error',
    })
    isLoading.value = false
  }
...
const resetCurrentInputDetection = (): void => {
  ...
  if (stepType?.inputType === 'axis') {
    const targetId = axisConfigMapROV[stepType.id]?.id
    if (targetId) {
      Object.entries(wizardJoystickMapping.value.axesCorrespondencies).some(([idx, entry]) => {
    ...
    const targetId = buttonConfigMapROV[stepType.id]?.id
    if (targetId) {
...
const overwriteConflictingMapping = (): void => {
  ...
  if (axisConfigMapROV[stepId] && detectedAxisId.value !== null) {
    const idx = detectedAxisId.value as JoystickAxis
    const { id, name, protocol } = axisConfigMapROV[stepId]
    const axisEntry = wizardJoystickMapping.value.axesCorrespondencies[idx]
    axisEntry.action = { id, name, protocol }
    axisEntry.min = -1000
    axisEntry.max = 1000
    ...
    if (stepId === 7) {
      ...
      const { id, name, protocol } = buttonConfigMapROV[stepId]
...
const detectInputs = (state: JoystickState): void => {
  ...
  state.buttons.forEach((isDown, idx) => {
    // Falling edge detection for the start button
    const wasDown = prevButtonStates.get(idx) ?? false

    if (!isDown && wasDown && !heldButtons.has(idx) && startButtonIndex.value === null) {
  ...
  if (stepMeta?.inputType === 'axis') {
    ...
    state.buttons.forEach((isDown, idx) => {
      ...
      const canDetect =
        isDown && detectedButtonId.value === null && idx !== shiftButtonIndex.value && !heldButtons.has(idx)
