  ...
  >
    <template #content>
      <div v-if="updateInfo" class="mt-2">
        ...
        <p>{{ $t('configuration.general.video.currentVersion') }}: {{ app_version.version }}</p>
        <p>{{ $t('configuration.general.video.newVersion') }}: {{ updateInfo.version }}</p>
        <p>{{ $t('configuration.general.video.releaseDate') }}: {{ formatDate(updateInfo.releaseDate) }}</p>
      </div>
      <v-progress-linear
        v-if="showProgress"
