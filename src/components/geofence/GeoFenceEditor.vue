  ...
  <div class="flex flex-col w-full">
        ...
        <template #content>
          <div class="flex flex-col items-center w-full px-1 py-1">
            ...
            <div
              ...
              @keydown.self.enter="onToggleShapeInteractive(polygon.id)"
              @keydown.self.space.prevent="onToggleShapeInteractive(polygon.id)"
            >
                ...
                <v-tooltip location="top" text="Delete polygon">
                  <template #activator="{ props: tooltipProps }">
                    <v-btn
                      ...
                      size="x-small"
                      density="comfortable"
                      @click.stop="onDeletePolygon(polygon.id)"
                    />
                  </template>
                </v-tooltip>
                <v-switch
        ...
        <template #content>
          <div class="flex flex-col items-center w-full px-1 py-1">
            ...
            <div
              ...
              @keydown.self.enter="onToggleShapeInteractive(circle.id)"
              @keydown.self.space.prevent="onToggleShapeInteractive(circle.id)"
            >
                ...
                <v-tooltip location="top" text="Delete circle">
                  <template #activator="{ props: tooltipProps }">
                    <v-btn
                      ...
                      size="x-small"
                      density="comfortable"
                      @click.stop="onDeleteCircle(circle.id)"
                    />
                  </template>
                </v-tooltip>
                <v-switch
    ...
    <div class="flex flex-col ma-2 py-2 px-4 border-[1px] border-[#FFFFFF22] bg-[#00000022] rounded-md p-2">
      <div v-if="fenceStore.isArduPilot" class="flex items-center justify-between gap-x-2 fence-autoenable-row">
        ...
        <v-switch
          :model-value="fenceStore.fenceAutoEnableMode !== undefined && fenceStore.fenceAutoEnableMode > 0"
          color="#3B78A8"
          density="compact"
          hide-details
          inset
          class="origin-right scale-[0.6] fence-autoenable-switch"
          :disabled="!vehicleStore.isVehicleOnline || fenceStore.fenceAutoEnableMode === undefined"
      ...
      <div v-if="fenceStore.breachReturn" class="flex items-center justify-between gap-x-2">
        ...
        <div class="flex items-center gap-x-1">
          ...
          />
          <v-tooltip location="top" text="Remove breach return point">
            <template #activator="{ props: tooltipProps }">
              <v-btn
                v-bind="tooltipProps"
                icon="mdi-delete"
...
// Sections of the last imported `.plan` that Cockpit does not edit, so exporting
...
const onToggleFenceAutoEnable = (value: boolean | null): void => {
  const target = value ? fenceStore.lastAutoEnableMode : 0
...
// Transferring shapes onto a vehicle that is already enforcing puts them into
// effect on arrival, and ArduPilot additionally needs the Polygon bit of
// `FENCE_TYPE` set for them to count, which the upload turns on by itself.
const confirmLiveEnforcementIfNeeded = async (): Promise<boolean> => {
  if (fenceStore.polygons.length === 0 && fenceStore.circles.length === 0) return true
  if (!(await fenceStore.readFenceEnabled())) return true
  return confirmAction(
    'Enforcement is already on',
    'This vehicle is enforcing a geofence right now, so it starts obeying the new shapes as soon as they reach it. ' +
      'If it is already outside a new inclusion area, the fence action configured on the autopilot (return, land ' +
      'or brake) fires immediately. Continue?',
    'Upload anyway'
  )
}
...
const confirmReplaceLocalPlanIfNeeded = async (): Promise<boolean> => {
  if (!fenceStore.hasItems) return true
  return confirmAction(
    'Replace local fence plan',
    'This replaces the polygons, circles and breach return point you are editing. Continue?',
    'Replace'
  )
}

const onDownload = async (): Promise<void> => {
  if (!(await confirmReplaceLocalPlanIfNeeded())) return
...
const onSaveCfp = (): void => {
  ...
  openSnackbar({ variant: 'success', message: 'Fence saved to file.', duration: 3000 })
}

const onExportMavlinkPlan = (): void => {
  logUserAction('Exported the local fence as a MAVLink `.plan` file')
  const planFile = geoFencePlanToMavlinkPlanFile(fenceStore.exportPlan(), importedPlanSections.value)
  const blob = new Blob([JSON.stringify(planFile, null, 2)], { type: 'application/json' })
  const date = format(new Date(), 'LLL_dd_yyyy_HH_mm_ss')
  saveAs(blob, `cockpit_fence_${date}.plan`)
  const keptMission = importedPlanSections.value.mission !== undefined
  const openWaypointsLeftOut = missionStore.currentPlanningWaypoints.length > 0
  if (!keptMission && !openWaypointsLeftOut) {
    openSnackbar({ variant: 'success', message: 'Fence exported as a plan file.', duration: 3000 })
    return
  }
  const parts = [
    keptMission ? 'the mission from the plan file you imported was kept unchanged' : null,
    openWaypointsLeftOut ? 'the waypoints open in mission planning were not added' : null,
  ].filter((part) => part !== null)
  openSnackbar({
    variant: keptMission ? 'info' : 'warning',
    message: `Fence exported as a plan file: ${parts.join(', and ')}.`,
    duration: 6000,
  })
}
...
/**
 * Loads the fence out of a `.plan` file, keeping its other sections for export.
 * @param { MavlinkPlanFile } planFile The parsed `.plan` document.
 * @returns { SnackbarOptions } The single message describing what was loaded.
 */
const importMavlinkPlan = (planFile: MavlinkPlanFile): SnackbarOptions => {
  const plan = mavlinkPlanFileToGeoFencePlan(planFile)
  if (plan !== undefined) fenceStore.loadFromPlan(plan)

  // Cockpit's fence editor does not own mission or rally-point editing, so
  // these sections are held untouched and written back on export instead of
  // being dropped from the file.
  importedPlanSections.value = { mission: planFile.mission, rallyPoints: planFile.rallyPoints }
  const keptNote =
    planFile.mission !== undefined ? ' The mission in it is kept and restored if you export it again.' : ''

  if (plan === undefined) {
    const reason =
      planFile.geoFence === undefined
        ? 'This plan file has no fence, so no fence was loaded.'
        : 'This plan file stores its fence in a format Cockpit cannot read, so no fence was loaded.'
    return { variant: 'warning', message: `${reason}${keptNote}`, duration: 5000 }
  }
  return { variant: 'success', message: `Fence loaded from the plan file.${keptNote}`, duration: 5000 }
}
...
const onFilePicked = async (event: Event): Promise<void> => {
  ...
  // Clearing the input on every exit is what lets the user pick the same file
  // again after backing out; otherwise no `change` event fires the second time.
  try {
    ...
    if (!instanceOfCockpitFencePlanFile(parsed) && !instanceOfMavlinkPlanFile(parsed)) {
    ...
    try {
      if (instanceOfCockpitFencePlanFile(parsed)) {
        ...
        openSnackbar(importMavlinkPlan(parsed))
      }
    } catch (error) {
      showDialog({ variant: 'error', message: 'Could not load the fence plan from this file.', timer: 3000 })
    }
  } finally {
    input.value = ''
  }
