<template>
  <v-tooltip location="top" text="Finish polygon fence">
    <template #activator="{ props: tooltipProps }">
      <div
        v-if="fenceStore.isDrawingPolygon && polygonVertexes.length >= 3"
        v-bind="tooltipProps"
  ...
  <v-tooltip location="top" text="Discard polygon fence">
    <template #activator="{ props: tooltipProps }">
      <div
        v-if="fenceStore.isDrawingPolygon && polygonVertexes.length >= 1"
        v-bind="tooltipProps"
        :style="confirmButtonStyle"
...
const updateConfirmButtonPosition = (): void => {
  ...
  if (fenceStore.isDrawingPolygon && props.polygonVertexes.length >= 3) {
...
watch([() => props.polygonVertexes, () => fenceStore.isDrawingPolygon], () => updateConfirmButtonPosition(), {
  immediate: true,
  deep: true,
})
