<template>
  <InteractionDialog
    ...
    :title="$t('components.GlobalOriginDialog.title')"
    variant="text-only"
    max-width="500px"
  ...
  >
    <template #content>
      <p class="text-sm mb-8 -mt-6 text-gray-300">
        {{ $t('components.GlobalOriginDialog.description') }}
      </p>

      <div class="flex items-center gap-2 mb-6">
        <v-text-field
          v-model.number="latitude"
          :label="$t('components.GlobalOriginDialog.latitude')"
          type="number"
          step="0.000001"
          :hint="$t('components.GlobalOriginDialog.latitudeHint')"
          persistent-hint
          variant="outlined"
          :rules="[
            (v: number) => v !== null && v !== undefined || $t('components.GlobalOriginDialog.latitudeRequired'),
            (v: number) => (v >= -90 && v <= 90) || $t('components.GlobalOriginDialog.latitudeRange'),
          ]"
          class="flex-1"
        />
      ...
      <div class="flex items-center gap-2 mb-6">
        <v-text-field
          ...
          :label="$t('components.GlobalOriginDialog.longitude')"
          type="number"
          step="0.000001"
          :hint="$t('components.GlobalOriginDialog.longitudeHint')"
          persistent-hint
          variant="outlined"
          :rules="[
            (v: number) => v !== null && v !== undefined || $t('components.GlobalOriginDialog.longitudeRequired'),
            (v: number) => (v >= -180 && v <= 180) || $t('components.GlobalOriginDialog.longitudeRange'),
          ]"
          class="flex-1"
        />
        <span class="text-white text-lg font-medium mb-6 w-4">°</span>
      ...
      <div class="flex items-center gap-2 mb-2">
        <v-text-field
          ...
          :label="$t('components.GlobalOriginDialog.altitude')"
          type="number"
          step="0.1"
          :hint="$t('components.GlobalOriginDialog.altitudeHint')"
          persistent-hint
          variant="outlined"
          :rules="[
            (v: number) => v !== null && v !== undefined || $t('components.GlobalOriginDialog.altitudeRequired'),
          ]"
          class="flex-1"
        />
...
const dialogActions = computed<Action[]>(() => [
  ...
  {
    text: isSaving.value ? t('components.GlobalOriginDialog.saving') : t('common.save'),
