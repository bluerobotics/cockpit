<template>
  <InteractionDialog
    v-model="isOpen"
    :title="
      searching ? $t('components.VehicleDiscoveryDialog.searching') : $t('components.VehicleDiscoveryDialog.title')
    "
  ...
  >
    <template #content>
      <div v-if="props.showAutoSearchOption && preventAutoSearch">
        <div class="text-sm mb-4">{{ $t('components.VehicleDiscoveryDialog.canStillSearch') }}</div>
      </div>
      <div v-else class="flex flex-col items-center justify-center gap-4 min-w-[300px] min-h-[100px]">
        <div v-if="searching" class="flex flex-col items-center gap-2 mb-2">
          ...
          <span v-if="vehicles.length === 0">{{ $t('components.VehicleDiscoveryDialog.searching_status') }}</span>
        ...
        <div v-if="vehicles.length > 0" class="flex flex-col gap-2 mb-3">
          <div v-if="!searching" class="h-4 font-weight-bold text-center mb-5">
            {{ $t('components.VehicleDiscoveryDialog.vehiclesFound') }}
          </div>
        ...
        <div v-else-if="searched && !searching" class="text-sm">
          {{ $t('components.VehicleDiscoveryDialog.noVehicles') }}
        </div>
        ...
        <div v-if="!searching && !searched" class="flex flex-col gap-2 items-center justify-center text-center">
          <p v-if="props.showAutoSearchOption" class="font-bold">
            {{ $t('components.VehicleDiscoveryDialog.notConnected') }}
          </p>
          <p class="max-w-[25rem] mb-2">
            {{ $t('components.VehicleDiscoveryDialog.description') }}
          </p>
        </div>
        ...
        <div v-if="!searching" class="flex justify-center items-center">
          <v-btn variant="outlined" :disabled="searching" class="mb-5" @click="searchVehicles">
            {{
              searched
                ? $t('components.VehicleDiscoveryDialog.searchAgain')
                : $t('components.VehicleDiscoveryDialog.searchButton')
            }}
          </v-btn>
        </div>
      </div>
...
if (props.showAutoSearchOption) {
  originalActions.unshift({
    text: t('components.VehicleDiscoveryDialog.dontShowAgain'),
