...
export const useVideoStore = defineStore('video', () => {
    ...
    Object.keys(activeStreams.value).forEach((streamName) => {
      ...
      if (oldStreamData && oldStreamData.webRtcManager) {
        if (isRecording(streamName)) {
          showDialog({ message: i18n.global.t('video.streamChanged', { streamName }), variant: 'error' })
  ...
  const startRecording = async (streamName: string): Promise<void> => {
    ...
    if (namesAvailableStreams.value.isEmpty()) {
      showDialog({ message: i18n.global.t('video.noStreamsAvailable'), variant: 'error' })
      return
    }

    if (activeStreams.value[streamName]!.mediaStream === undefined) {
      showDialog({ message: i18n.global.t('video.mediaStreamNotDefined'), variant: 'error' })
      return
    }
    if (!activeStreams.value[streamName]!.mediaStream!.active) {
      showDialog({ message: i18n.global.t('video.mediaStreamNotActive'), variant: 'error' })
      return
    }
    ...
    if (window.electronAPI) {
      ...
      recordingMonitors[streamName] = setInterval(async () => {
        ...
        if (!activeStreams.value[streamName] || !activeStreams.value[streamName]!.mediaRecorder) {
          const msg = i18n.global.t('video.recordingStoppedForStream', { streamName })
        ...
        if (!fileStats || !fileStats.exists) {
          ...
          const msg = i18n.global.t('video.cannotGetVideoFileSize')
        ...
        if (fileStats.size! <= lastKnownFileSize!) {
          const msg = i18n.global.t('video.videoFileNotGrowing')
          showDialog({ message: msg, variant: 'error' })
          return
        }
      ...
      recordingMonitors[streamName] = setInterval(async () => {
        ...
        if (numberOfChunks <= lastKnownNumberOfChunks!) {
          const msg = i18n.global.t('video.videoChunksNotGrowing')
          showDialog({ message: msg, variant: 'error' })
          return
        }
    ...
    activeStreams.value[streamName]!.mediaRecorder!.ondataavailable = async (e) => {
      ...
      try {
        ...
        if (processor && e.data.size > 0) {
          try {
            ...
            if (error instanceof LiveVideoProcessorChunkAppendingError) {
              ...
              const msg = i18n.global.t('video.failedToAddChunk', { chunksCount, error: error.message })
              openSnackbar({ message: msg, variant: 'error' })
            } else if (error instanceof LiveVideoProcessorInitializationError) {
              const msg = i18n.global.t('video.failedToInitializeProcessor', { streamName, error: error.message })
              showDialog({ message: msg, variant: 'error' })
        ...
        if (chunksCount === 0) {
          const msg = i18n.global.t('video.failedToInitiateRecording')
          showDialog({ message: msg, variant: 'error' })
          alertStore.pushAlert(new Alert(AlertLevel.Error, msg))
          stopRecording(streamName)
        }
    ...
    activeStreams.value[streamName]!.mediaRecorder!.onstop = async () => {
      ...
      try {
        ...
        openSnackbar({ message: i18n.global.t('video.failedToGenerateTelemetryOverlay'), variant: 'error' })
      }
  ...
  const deleteStreamCorrespondency = (externalId: string): void => {
    ...
    if (streamIndex !== -1) {
      ...
      openSnackbar({ variant: 'success', message: i18n.global.t('video.streamDeletedAndIgnored', { streamName: stream.name }) })
    } else {
      openSnackbar({ variant: 'warning', message: i18n.global.t('video.streamNotFound', { externalId }) })
    }
  }
  ...
  const restoreIgnoredStream = (externalId: string): void => {
    ...
    if (ignoredIndex !== -1) {
      ...
      if (namesAvailableStreams.value.includes(externalId)) {
        ...
        openSnackbar({ variant: 'warning', message: i18n.global.t('video.streamNotAvailable', { externalId }) })
      }

      openSnackbar({ variant: 'success', message: i18n.global.t('video.streamRestored', { externalId }) })
    } else {
      openSnackbar({ variant: 'warning', message: i18n.global.t('video.streamNotInIgnoredList', { externalId }) })
    }
  }
