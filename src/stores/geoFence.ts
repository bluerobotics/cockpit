...
export const useGeoFenceStore = defineStore('geo-fence', () => {
  ...
  // The live overlay and the enforcement control both key off this cache, so it
  // only ever holds geometry: a fence with no shapes is the same as no fence, and
  // caching one would arm the enforcement toggle with nothing to enforce.
  const cacheVehicleFence = (plan: GeoFencePlan | undefined): void => {
    if (!planHasShapes(plan)) {
  ...
  if (planHasShapes(persistedVehicleFence.value)) {
  ...
  watch(persistedVehicleFence, (plan) => {
    if (!planHasShapes(plan) || lastUploadedPlan.value) return
    lastUploadedPlan.value = clonePlan(plan)
  })
  ...
  /**
   * Reads the vehicle's current `FENCE_ENABLE` value, awaiting the PARAM_VALUE
   * reply. Needed by callers that must know the live enforcement state right
   * now: the polled `fenceEnabled` is only kept warm while a fence is already
   * cached for the vehicle, so it reads `undefined` before the first upload.
   * @returns { Promise<boolean | undefined> } Enforcement state, or `undefined` when the vehicle did not answer.
   */
  const readFenceEnabled = async (): Promise<boolean | undefined> => {
    const v = mainVehicleStore.mainVehicle
    if (!v || !isArduPilot.value) return undefined
    const value = await v.requestParameterValue('FENCE_ENABLE')
    return value === undefined ? undefined : Math.round(value) === 1
  }
  ...
  const announceFetchedFence = (plan: GeoFencePlan, successMessage: string): void => {
    openSnackbar(
      planHasShapes(plan)
        ? { variant: 'success', message: successMessage, duration: 3000 }
        : { variant: 'info', message: 'No geofence is stored on the vehicle.', duration: 3000 }
    )
  }
  ...
  const refreshVehicleFenceOverlay = async (loadingCallback?: MissionLoadingCallback): Promise<void> => {
    ...
    announceFetchedFence(plan, 'Geofence overlay refreshed from vehicle.')
  }
  ...
  const downloadFromVehicle = async (loadingCallback?: MissionLoadingCallback): Promise<void> => {
    ...
    loadFromPlan(plan)
    dirty.value = false
    announceFetchedFence(plan, 'Geofence downloaded from vehicle.')
  }
