...
export const useWidgetManagerStore = defineStore('widget-manager', () => {
  ...
  const showElementPropsDrawer = (customWidgetElementHash: string): void => {
    ...
    if (!customWidgetElement) {
      openSnackbar({ variant: 'error', message: i18n.global.t('stores.widgetManager.elementNotFound'), duration: 3000 })
      return
    }
  ...
  const loadWidgetFromFile = (widgetHash: string, loadedWidget: Widget): void => {
    ...
    if (widgetIndex === -1) {
      openSnackbar({ variant: 'error', message: i18n.global.t('stores.widgetManager.widgetNotFound'), duration: 3000 })
      return
    }
    ...
    openSnackbar({
      variant: 'success',
      message: i18n.global.t('stores.widgetManager.widgetLoadedSuccessfully'),
      duration: 3000,
    })
  }
  ...
  const currentProfile = computed<Profile>({
    ...
    set(newValue) {
      ...
      if (!profilesHashes.includes(newValue.hash)) {
        showDialog({ variant: 'error', message: i18n.global.t('stores.widgetManager.profileNotFound'), timer: 3000 })
        return
      }
  ...
  function loadProfile(profile: Profile): void {
    ...
    if (profileIndex === -1) {
      showDialog({ message: i18n.global.t('stores.widgetManager.profileNotFound'), variant: 'error', timer: 3000 })
      return
    }
  ...
  function renameView(view: View, name: string): void {
    ...
    if (name.length === 0) {
      showDialog({
        variant: 'error',
        message: i18n.global.t('stores.widgetManager.viewNameCannotBeBlank'),
        timer: 2000,
      })
      return
    }
  ...
  const selectView = (view: View): void => {
    if (!view.visible) {
      showDialog({
        variant: 'error',
        message: i18n.global.t('stores.widgetManager.cannotSelectInvisibleView'),
        timer: 5000,
      })
      return
    }
  ...
  function deleteMiniWidget(miniWidget: MiniWidget): void {
    ...
    showDialog({ variant: 'error', message: i18n.global.t('stores.widgetManager.miniWidgetContainerNotFound') })
  }
