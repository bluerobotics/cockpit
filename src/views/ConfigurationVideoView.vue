<template>
    ...
    <template #content>
      <div class="flex-col h-full ml-[1vw] w-[840px] max-h-[85vh] overflow-y-auto pr-3">
            ...
            <div class="flex justify-center flex-col w-full ml-2 mt-2">
              ...
              >
                ...
                <template #no-data>
                  <div class="text-gray-400 py-4 w-[200px] text-end">
                    {{ $t('configuration.video.noStreamsFound') }}
                  </div>
                </template>
              ...
              <div class="flex items-center justify-start">
                <v-checkbox
                  ...
                  :label="$t('configuration.video.showIgnoredStreams')"
                  hide-details
                  class="text-sm"
                />
        ...
        <ExpansiblePanel no-bottom-divider :is-expanded="!interfaceStore.isOnPhoneScreen">
          ...
          <template #info>
            {{ $t('configuration.video.videoLibraryOptionsInfo1') }}
            <br /><br />
            {{ $t('configuration.video.videoLibraryOptionsInfo2') }}
            <br /><br />
            {{ $t('configuration.video.videoLibraryOptionsInfo3') }}
          </template>
          <template #content>
            ...
            <div class="flex items-center justify-start w-[96%] ml-2">
              ...
              <v-tooltip max-width="400px">
                ...
                <div class="text-sm">
                  ...
                  <p class="mb-2">
                    {{ $t('configuration.video.saveBackupRawChunksEnabled') }}
                  </p>
                  <p>
                    {{ $t('configuration.video.saveBackupRawChunksDisabled') }}
                  </p>
  ...
  <InteractionDialog
    ...
    :title="$t('configuration.general.video.editStream')"
    variant="text-only"
    :persistent="true"
    :actions="[
    ...
    <template #content>
      <div class="flex flex-col gap-6 px-4 mb-6">
        <div class="text-sm text-gray-400">
          <span>{{ $t('videoStream.externalStreamName') }}: </span>
          <span class="text-gray-200">{{ editingStream?.externalId }}</span>
        </div>
        <v-text-field
          v-model="newStreamName"
          :label="$t('videoStream.internalStreamName')"
          variant="outlined"
          density="compact"
          hide-details
          autofocus
  ...
  <InteractionDialog
    ...
    :title="$t('configuration.general.video.streamNotAvailable')"
    variant="text-only"
    :persistent="true"
    max-width="520px"
    :actions="[
      { text: $t('configuration.general.video.keepIgnored'), size: 'small', action: closeUnavailableStreamDialog },
      { text: $t('configuration.general.video.deletePermanently'), size: 'small', action: deleteStreamPermanently },
    ]"
  >
    <template #content>
      <div class="flex flex-col gap-4 px-4 mb-6">
        <p class="text-sm text-gray-300">
          {{ $t('configuration.general.video.streamNotAvailableMessage').replace('{streamId}', `'${unavailableStreamId}'`) }}
        </p>
        <p class="text-sm text-gray-300">{{ $t('configuration.general.video.twoOptions') }}</p>
        <ul class="text-sm text-gray-300 ml-4 space-y-1">
          <li>
            • {{ $t('configuration.general.video.keepIgnoredDesc') }}
          </li>
          <li>
            • {{ $t('configuration.general.video.deletePermanentlyDesc') }}
          </li>
        </ul>
      </div>
    </template>
