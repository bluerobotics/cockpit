<template>
  <BaseConfigurationView>
    <template #title>{{ $t('User Interface') }}</template>
    <template #content>
      <div class="max-h-[85vh] overflow-y-auto">
        <ExpansiblePanel no-top-divider :is-expanded="!interfaceStore.isOnPhoneScreen">
          <template #title>{{ $t('Window Material') }}</template>
          <template #content>
            <div class="flex w-full">
              <div class="flex flex-col w-full px-4 pt-5">
                <div class="flex flex-row justify-start items-center w-full mb-[35px] gap-x-[85px]">
                    ...
                    >
                      <template #activator="{ props }">
                        <div v-bind="props" class="flex cursor-pointer gap-x-[30px]">
                          <span class="text-start mt-[2px]">{{ $t('Glass color') }}</span>
                          <div
                    ...
                    >
                      <template #activator="{ props }">
                        <div v-bind="props" class="flex gap-x-[30px]">
                          <span class="text-start mt-[2px]">{{ $t('Font color') }}</span>
                          <div
                            v-bind="props"
                  ...
                  <v-btn variant="text" size="small" @click="resetColorsToDefault">{{ $t('Reset to defaults') }}</v-btn>
                </div>
                <div class="flex w-full">
                  <div class="flex w-[33%] mt-[2px]">{{ $t('Opacity') }}</div>
                  <div class="flex w-[66%]">
                    <v-slider
                ...
                <div class="flex w-full">
                  <div class="flex w-[33%] mt-[2px]">{{ $t('Blur') }}</div>
                  <div class="flex w-[66%]">
                    <v-slider
            ...
            <div class="flex w-full">
              <div class="flex flex-col w-full px-4 pt-5">
                <div class="flex flex-row justify-start items-center w-full mb-[35px]">
                  <div class="flex w-[33%]">{{ $t('Main menu trigger position') }}</div>
                  <div class="flex w-[66%]">
                    <v-radio-group v-model="interfaceStore.mainMenuStyleTrigger" inline hide-details>
                      <v-radio :label="$t('Center-left tab')" value="center-left" />
                      <v-radio :label="$t('Top bar button')" value="burger" class="ml-6" />
                    </v-radio-group>
                  </div>
                </div>
              </div>
        ...
        <ExpansiblePanel no-bottom-divider :is-expanded="!interfaceStore.isOnPhoneScreen">
          <template #title>{{ $t('Display Units') }}</template>
          <template #content>
            <div class="flex w-full">
              <div class="flex flex-col w-full px-4 pt-5">
                <div class="flex flex-row justify-start items-center w-full mb-[35px]">
                  <div class="flex w-[33%]">{{ $t('Distance') }}</div>
                  <div class="flex w-[66%]">
                    <v-radio-group v-model="interfaceStore.displayUnitPreferences.distance" inline hide-details>
                      <v-radio :label="$t('Meters')" :value="DistanceDisplayUnit.Meters" />
                      <v-radio :label="$t('Feet')" :value="DistanceDisplayUnit.Feet" class="ml-6" />
                    </v-radio-group>
                  </div>
                </div>
              </div>
