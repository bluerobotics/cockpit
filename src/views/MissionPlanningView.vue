      ...
      <div class="flex flex-col w-full h-full p-2 overflow-y-auto">
        ...
        <div>
          <div class="flex w-full justify-between my-2 px-1">
            ...
            <v-divider vertical />
            <v-tooltip location="top" :text="t('Save mission to file')">
              <template #activator="{ props }">
                <v-btn
                  v-bind="props"
            ...
            <v-divider vertical />
            <v-tooltip location="top" :text="t('Load mission from file')">
              <template #activator="{ props }">
                <v-btn
                  v-bind="props"
                  icon="mdi-upload"
        ...
        >
          {{ $t('Upload to the vehicle').toUpperCase() }}
        </button>
        <button
...
const downloadMissionFromVehicle = async (): Promise<void> => {
  ...
  try {
    ...
    openSnackbar({
      variant: 'success',
      message: t('Mission download succeeded!'),
      duration: 3000,
    })
  } catch (error) {
    ...
    showDialog({
      variant: 'error',
      title: t('Mission download failed'),
...
const pickBestPosition = (
  ...
  for (const c of candidates) {
    ...
    const visH = Math.max(0, Math.min(b, vpH - margin) - Math.max(top, margin))
...
const setHomePosition = async (): Promise<void> => {
  ...
  try {
    ...
    openSnackbar({
      variant: 'success',
      message: t('Home position set to {lat}, {lon}', {
    ...
    openSnackbar({
      variant: 'error',
      message: t('Failed to set home position: {error}', { error: String(error) }),
    })
  }
}
...
const deleteSelectedSurvey = (): void => {
  ...
  if (surveyIndex === -1) {
    openSnackbar({
      variant: 'error',
      message: t('Selected survey does not exist.'),
      duration: 2000,
    })
    return
  }
    ...
    reader.onload = (e: ProgressEvent<FileReader>): void => {
      try {
        ...
        if (typeof contents !== 'string') {
          showDialog({ variant: 'error', message: t('File does not appear to be in the correct format.'), timer: 3000 })
          return
        }
        const maybeMission = JSON.parse(contents)
        if (!instanceOfCockpitMission(maybeMission)) {
          showDialog({ variant: 'error', message: t('Invalid mission file.'), timer: 3000 })
          return
        }
        ...
        showDialog({ variant: 'error', message: `${t('Failed to load mission file:')} ${error}`, timer: 5000 })
      }
    }
...
const createSurveyPath = (): void => {
  ...
  try {
    ...
    if (result.path.length === 0) {
      showDialog({
        variant: 'error',
        message: t('No valid survey path'),
        timer: 5000,
      })
      return
    }
    ...
    showDialog({
      variant: 'error',
      message: t('Failed to generate survey path: {error}', { error: (error as Error).message }),
      timer: 5000,
    })
  }
}
...
const generateWaypointsFromSurvey = (): void => {
  ...
  openSnackbar({
    variant: 'success',
    message: t('Waypoints generated from survey path.'),
    duration: 1000,
  })
}
...
const regenerateSurveyWaypoints = (angle?: number): void => {
  ...
  if (selectedSurvey.value) {
    ...
    if (firstOldWaypointIndex === -1) {
      openSnackbar({
        variant: 'error',
        message: t('Failed to find old waypoints.'),
        duration: 2000,
      })
      return
    }
...
const loadDraftMission = async (mission: CockpitMission): Promise<void> => {
  ...
  try {
    ...
    openSnackbar({
      variant: 'error',
      message: t('Failed to load draft mission: {error}', { error: String(error) }),
      duration: 3000,
    })
  }
}
...
const centerMissionButtonTooltipText = computed(() => {
  if (!hasMissionWaypoints.value) {
    return t('Cannot center map on mission (no waypoints defined).')
  }
...
const openPoiDialog = (): void => {
  if (cursorCoordinates.value && poiManagerRef.value) {
    ...
    showDialog({
      variant: 'error',
      title: t('Error'),
      message: t('POI Manager (map widget) is not available.'),
    })
...
const addPoiMarkerToPlanningMap = (poi: PointOfInterest): void => {
  ...
  const tooltipContent = `
    ...
    ${t('Lat: {lat}, Lng: {lng}', {
      lat: poi.coordinates[0].toFixed(8),
      lng: poi.coordinates[1].toFixed(8),
    })}
  `
  ...
  marker.on('drag', (event) => {
    ...
    const updatedTooltipContent = `
      <strong>${poi.name}</strong><br>
      ${poi.description ? poi.description + '<br>' : ''}
      ${t('Lat: {lat}, Lng: {lng}', { lat: newCoords.lat.toFixed(8), lng: newCoords.lng.toFixed(8) })}
    `
