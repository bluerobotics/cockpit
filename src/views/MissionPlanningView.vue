...
const onPolygonMouseUp = (event: L.LeafletMouseEvent): void => {
  ...
  // // If it was a drag, don't process it as a click for POI or other map click actions
  // if (
  //   Math.abs(event.latlng.lat - (dragStartLatLng?.lat ?? 0)) > 1e-6 ||
  //   Math.abs(event.latlng.lng - (dragStartLatLng?.lng ?? 0)) > 1e-6
  // ) {
