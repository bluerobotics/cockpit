<template>
  ...
  <div
    ...
    :style="interfaceStore.globalGlassMenuStyles"
  >
    <p>{{ $t('missionPlanning.savingOfflineMapContent', { layerName: savingLayerName }) }}:&nbsp;{{ tilesTotal ? Math.round((tilesSaved / tilesTotal) * 100) : 0 }}%</p>
  </div>
...
const timeAgoSeenText = computed(() => {
  ...
  if (!lastBeat) return t('map.never')
...
watch([vehiclePosition, vehicleHeading, timeAgoSeenText, () => vehicleStore.isArmed], () => {
  ...
  vehicleMarker.value.getTooltip()?.setContent(`
    <p>${t('map.coordinates')}: ${vehiclePosition.value?.[0].toFixed(6)}, ${vehiclePosition.value?.[1].toFixed(6)}</p>
    <p>${t('map.velocity')}: ${vehicleStore.velocity.ground?.toFixed(2) ?? 'N/A'} m/s</p>
    <p>${t('map.heading')}: ${vehicleHeading.value.toFixed(2)}°</p>
    <p>${vehicleStore.isArmed ? t('map.armed') : t('map.disarmed')}</p>
    <p>${t('map.lastSeen')}: ${timeAgoSeenText.value}</p>
  `)
...
const openPoiDialog = (): void => {
  if (cursorCoordinates.value && poiManagerRef.value) {
    ...
    showDialog({ variant: 'error', title: t('common.error'), message: t('map.cannotPlacePOI') })
    console.error('Cannot open POI dialog without click coordinates for new POI')
  } else if (!poiManagerRef.value) {
    showDialog({ variant: 'error', title: t('common.error'), message: t('map.poiManagerNotAvailable') })
