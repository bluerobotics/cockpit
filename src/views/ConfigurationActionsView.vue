<template>
  <BaseConfigurationView>
    <template #title>{{ $t('configuration.actions.title') }}</template>
    <template #content>
      <div class="flex-col h-full overflow-y-auto ml-[10px] pr-3 -mr-[10px] -mb-[10px]">
        <ExpansiblePanel no-top-divider no-bottom-divider :is-expanded="!interfaceStore.isOnPhoneScreen">
          <template #title>{{ $t('configuration.actions.actions') }}</template>
          <template #info>
            <li>{{ $t('configuration.actions.info1') }}</li>
            <li>{{ $t('configuration.actions.info2') }}</li>
            <li>{{ $t('configuration.actions.info3') }}</li>
            <li>{{ $t('configuration.actions.info4') }}</li>
            <li>{{ $t('configuration.actions.info5') }}</li>
            <li>{{ $t('configuration.actions.info6') }}</li>
          </template>
          <template #content>
                  ...
                  <tr class="w-full">
                    <td colspan="3" class="text-center flex items-center justify-center h-[50px] mb-3 w-full gap-2">
                      <v-btn variant="outlined" class="rounded-lg" @click="actionTypeDialog.show = true">
                        ...
                        {{ $t('configuration.actions.newAction') }}
                      </v-btn>
                      <v-btn variant="outlined" class="rounded-lg" @click="importAction">
                        <v-icon start>mdi-import</v-icon>
                        {{ $t('configuration.actions.importAction') }}
                      </v-btn>
                    </td>
                  </tr>
                </template>
                <template #no-data>
                  <tr>
                    <td colspan="3" class="text-center flex items-center justify-center h-[50px] w-full">
                      <p class="text-[16px] ml-[170px] w-full">{{ $t('configuration.actions.noActionsFound') }}</p>
                    </td>
                  </tr>
                </template>
              </v-data-table>
        ...
        <v-dialog v-model="actionTypeDialog.show" max-width="400px">
          <v-card class="rounded-lg" :style="interfaceStore.globalGlassMenuStyles">
            <v-card-title class="text-h6 font-weight-bold py-4 text-center">{{ $t('configuration.actions.selectActionType') }}</v-card-title>
            ...
            <v-card-actions>
              <div class="flex justify-between items-center pa-2 w-full h-full">
                <v-btn color="white" variant="text" @click="actionTypeDialog.show = false">{{ $t('configuration.actions.cancel') }}</v-btn>
              </div>
            </v-card-actions>
          </v-card>
        </v-dialog>
...
const getActionTypeName = (type: customActionTypes): string => {
  const typeMap = {
    [customActionTypes.httpRequest]: 'httpRequest',
    [customActionTypes.mavlinkMessage]: 'mavlinkMessage',
    [customActionTypes.javascript]: 'javascript',
  }
  return t(`configuration.actions.actionTypes.${typeMap[type]}`)
}

const headers = [
  { title: t('configuration.actions.name'), key: 'name', sortable: true, align: 'start' },
  { title: t('configuration.actions.type'), key: 'type', sortable: true, align: 'center' },
  { title: t('configuration.actions.minInterval'), key: 'minInterval', sortable: false, align: 'center' },
  { title: t('configuration.actions.linkedVariables'), key: 'linkedVariables', sortable: false, align: 'center' },
  { title: t('configuration.actions.actionsColumn'), key: 'actions', sortable: false, align: 'end' },
]
...
const actionTypes = [
  {
    title: t('configuration.actions.httpRequestAction'),
    description: t('configuration.actions.httpRequestDescription'),
    value: customActionTypes.httpRequest,
    icon: 'mdi-web',
  },
  {
    title: t('configuration.actions.mavlinkMessageAction'),
    description: t('configuration.actions.mavlinkMessageDescription'),
    value: customActionTypes.mavlinkMessage,
    icon: 'mdi-drone',
  },
  {
    title: t('configuration.actions.javascriptAction'),
    description: t('configuration.actions.javascriptDescription'),
