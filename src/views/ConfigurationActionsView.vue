<template>
  <BaseConfigurationView>
    <template #title>{{ $t('Cockpit actions configuration') }}</template>
    <template #content>
      <div class="flex-col h-full overflow-y-auto ml-[10px] pr-3 -mr-[10px] -mb-[10px]">
        <ExpansiblePanel no-top-divider no-bottom-divider :is-expanded="!interfaceStore.isOnPhoneScreen">
          <template #title>{{ $t('Cockpit actions configuration') }}</template>
          <template #info>
            <li>{{ $t('View, manage, and create different types of actions.') }}</li>
            <li>
              {{ $t('HTTP Request actions can be used to call external APIs, like servers, vehicles, cameras, etc.') }}
            </li>
            <li>{{ $t('MAVLink Message actions allow you to send specific MAVLink messages to vehicles.') }}</li>
            <li>{{ $t('JavaScript actions give you full flexibility by allowing you to write custom code.') }}</li>
            <li>
              {{
                $t(
                  'The link button can be used to link Actions to Data Lake variables, so that changes to the variable value will automatically trigger the linked action.'
                )
              }}
            </li>
            <li>{{ $t('Actions can also be tested/run manually, using the play button.') }}</li>
          </template>
          <template #content>
          ...
          <v-card class="rounded-lg" :style="interfaceStore.globalGlassMenuStyles">
            ...
            <v-card-actions>
              <div class="flex justify-between items-center pa-2 w-full h-full">
                <v-btn color="white" variant="text" @click="actionTypeDialog.show = false">{{ $t('Cancel') }}</v-btn>
              </div>
            </v-card-actions>
          </v-card>
        </v-dialog>
...
const translateActionType = (type: customActionTypes): string => {
  const typeMap: Record<customActionTypes, string> = {
    [customActionTypes.httpRequest]: t('HTTP Request'),
    [customActionTypes.mavlinkMessage]: t('MAVLink Message'),
    [customActionTypes.javascript]: t('JavaScript'),
  }
...
const headers = [
  { title: t('Name'), key: 'name', sortable: true, align: 'start' },
  { title: t('Type'), key: 'type', sortable: true, align: 'center' },
  { title: t('Min Interval'), key: 'minInterval', sortable: false, align: 'center' },
  {
    title: t('Linked Variables'),
    key: 'linkedVariables',
    sortable: false,
    align: 'center',
  },
...
const actionTypes = computed(() => [
  {
    title: t('HTTP Request Action'),
    description: t('Create an action to make HTTP requests to external APIs and services'),
    value: customActionTypes.httpRequest,
    icon: 'mdi-web',
  },
  {
    title: t('MAVLink Message Action'),
    description: t('Create an action to send MAVLink messages to vehicles'),
    value: customActionTypes.mavlinkMessage,
    icon: 'mdi-drone',
  },
  {
    title: t('JavaScript Action'),
    description: t('Create a custom action using JavaScript code'),
