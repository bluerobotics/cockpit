    ...
    <template #content>
      ...
      >
        <ExpansiblePanel no-top-divider no-bottom-divider :is-expanded="!interfaceStore.isOnPhoneScreen">
          ...
          <template #info>
            <p class="w-full">
              ...
              <br />
              <br />
              {{ $t('configuration.general.pirateModeInfo') }}
            </p>
          </template>
          <template #content>
            <div class="flex flex-col w-full items-start">
              ...
              <div class="flex flex-row w-full items-center justify-between py-3 gap-x-2">
                ...
                <v-select
                  ...
                  hide-details
                  flat
                  class="w-40 language-select"
                  bg-color="transparent"
                  base-color="white"
                  color="white"
                  :menu-props="{
                    class: 'glass-menu-dropdown',
                    contentClass: 'glass-menu-content',
                  }"
                ...
                >
                  {{
                    interfaceStore.pirateMode
                      ? $t('configuration.general.disablePirateMode')
                      : $t('configuration.general.enablePirateMode')
                  }}
                </v-btn>
              </div>
            </div>
        ...
        <ExpansiblePanel :is-expanded="!interfaceStore.isOnPhoneScreen">
          ...
          <template #subtitle
            >{{ $t('configuration.general.currentAddress') }} {{ mainVehicleStore.globalAddress }}</template
          >
              ...
              >
                ...
                >
                  <template #append-inner>
                    <v-icon v-tooltip.bottom="$t('configuration.interface.resetGlobalAddress')" color="white" @click="resetGlobalAddress">
                      mdi-refresh
                    </v-icon>
                  </template>
                </v-text-field>
        ...
        <ExpansiblePanel no-top-divider :is-expanded="!interfaceStore.isOnPhoneScreen">
          ...
          <template #subtitle>
            {{ $t('configuration.general.currentAddress') }}
            {{ ConnectionManager.mainConnection()?.uri().toString() ?? 'none' }}<br />
            {{ $t('configuration.general.status') }}
            {{
              vehicleConnected
                ? $t('configuration.general.connected')
                : vehicleConnected === undefined
                ? $t('configuration.general.connecting')
                : $t('configuration.general.failedToConnect')
            }}
          </template>
          <template #content>
              ...
              <div class="flex flex-row w-full justify-start gap-x-8 align-center">
                ...
                >
                  ...
                  >
                    <template #append-inner>
                      <v-icon
                        v-tooltip.bottom="$t('configuration.interface.resetToDefault')"
                        color="white"
                        :disabled="!mainVehicleStore.customMAVLink2RestWebsocketURI.enabled"
              ...
              <div class="flex justify-end mt-6">
                ...
                >
                  <v-switch
                    ...
                    v-tooltip.bottom="$t('configuration.interface.enableCustom')"
                  ...
                  <div class="-mt-[4px]">
                    {{
                      mainVehicleStore.customMAVLink2RestWebsocketURI.enabled
                        ? $t('configuration.general.enabled')
                        : $t('configuration.general.disabled')
                    }}
                  </div>
                </div>
              </div>
            ...
            >
              ...
              <div>
                ...
                >
                  ...
                  <div class="-mt-[4px]">
                    {{
                      mainVehicleStore.customWebRTCSignallingURI.enabled
                        ? $t('configuration.general.enabled')
                        : $t('configuration.general.disabled')
                    }}
                  </div>
            ...
            <div class="flex justify-between mt-2 w-full">
              ...
              <div class="flex flex-col justify-around align-center w-[100px] -mr-6">
                ...
                <div class="flex flex-col align-end text-[10px] -mt-8">
                  ...
                  <div class="-mt-[4px]">
                    {{
                      mainVehicleStore.customWebRTCConfiguration.enabled
                        ? $t('configuration.general.enabled')
                        : $t('configuration.general.disabled')
                    }}
                  </div>
...
const changeLanguage = (newLocale: string): void => {
  ...
  openSnackbar({ message: t('success.languageChanged'), variant: 'success', duration: 3000 })
}
...
<style>
/* Global glass menu dropdown styles (not scoped) */
.glass-menu-dropdown.v-overlay {
  background: transparent !important;
}

.glass-menu-dropdown .v-overlay__content {
...
.glass-menu-content.v-list,
.glass-menu-dropdown .v-list {
  background: transparent !important;
}

.glass-menu-dropdown .v-list-item {
  color: white !important;
}

.glass-menu-dropdown .v-list-item:hover {
  background: rgba(255, 255, 255, 0.15) !important;
}

.glass-menu-dropdown .v-list-item--active {
