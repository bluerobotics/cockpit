<template>
  <div class="mission-planning">
    ...
    >
      <WindDirectionDial v-model:angle="surveyLinesAngle" polygon-state auto-update />
    </div>
    ...
    >
      <div class="flex flex-col w-full h-full p-2 overflow-y-auto">
        ...
        <div v-if="isCreatingSurvey" class="flex flex-col">
          <p class="m-1 overflow-visible text-sm text-slate-200">Scan density (m)</p>
          <input
            v-model.number="distanceBetweenSurveyLines"
    ...
    <div
      v-if="selectedSurvey && selectedSurvey.waypoints.length > 0"
      class="absolute right-8 m-4 rounded-[16px] max-h-[70%] w-52 overflow-y-auto"
    ...
    >
      <ExpansiblePanel
        ...
        no-top-divider
        compact
        class="flex ex-col w-full max-h-[69%] p-2 overflow-y-auto"
      >
        <template #title
          ><p class="flex w-full justify-center">{{ selectedSurvey?.waypoints.length }} Waypoints:</p></template
        >
        <template #content>
          <p v-if="selectedSurvey?.waypoints.length === 0" class="text-lg text-center text-slate-100">
            No waypoints added to the mission.
          </p>
          <div
            v-for="(waypoint, index) in selectedSurvey?.waypoints"
            :key="waypoint.id"
          ...
          >
            ...
            <div
              v-if="selectedSurvey && index !== selectedSurvey.waypoints.length - 1"
...
const regenerateSurveyWaypoints = (angle?: number): void => {
  ...
  showSnackbar({ variant: 'success', message: 'Waypoints regenerated with new angle.', duration: 1000 })
}
...
const generateWaypointsFromSurvey = (): void => {
  ...
  lastSurveyState.value[newSurveyId] = {
    polygonPositions: [...surveyPolygonVertexesPositions.value],
    polygonMarkers: [...(surveyPolygonVertexesMarkers.value as L.Marker<any>[])],
    edgeMarkers: [...(surveyEdgeAddMarkers as L.Marker<any>[])],
...
const undoGenerateWaypoints = (): void => {
  ...
  surveyPolygonVertexesPositions.value = surveyState.polygonPositions
  surveyPolygonVertexesMarkers.value = []

  surveyState.polygonMarkers.forEach((marker) => {
    marker.addTo(planningMap.value!)
    surveyPolygonVertexesMarkers.value.push(marker)
  })

  surveyEdgeAddMarkers.length = 0
  surveyState.edgeMarkers.forEach((marker) => {
    marker.addTo(planningMap.value!)
    surveyEdgeAddMarkers.push(marker)
  })
  ...
  surveyLinesAngle.value = surveyState.surveyLinesAngle
  distanceBetweenSurveyLines.value = surveyState.distanceBetweenLines
...
provide('toggleSimpleMission', toggleSimpleMission)
provide('toggleSurvey', toggleSurvey)
provide('isCreatingSimpleMission', isCreatingSimpleMission)
provide('isCreatingSurvey', isCreatingSurvey)
provide('selectedSurveyId', selectedSurveyId)
provide('surveyLinesAngle', (angle: number) => {
  surveyLinesAngle.value = angle
})
provide('undoIsInProgress', undoIsInProgress)
provide('undoGenerateWaypoints', undoGenerateWaypoints)
provide('deleteSelectedSurvey', deleteSelectedSurvey)
