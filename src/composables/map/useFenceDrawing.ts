  ...
  const updatePolygonLayer = (): void => {
    ...
    if (polygonLayer.value) {
      ...
      polygonLayer.value = L.polygon(polygonVertexesPositions.value, {
        ...
        weight: 2,
        dashArray: '4 4',
        className: `fence-polygon-draft ${FENCE_PATH_CLASS}`,
      }).addTo(map)
    }
  ...
  const updatePendingCircleLayer = (): void => {
    ...
    if (!center || radius < 1) {
      if (pendingCircleLayer.value) {
        map.removeLayer(pendingCircleLayer.value as unknown as L.Layer)
    ...
    if (pendingCircleLayer.value) {
      ...
      pendingCircleLayer.value = L.circle(latlng, {
        ...
        weight: 2,
        dashArray: '4 4',
        className: `fence-circle-draft ${FENCE_PATH_CLASS}`,
        interactive: false,
      }).addTo(map)
    }
  }
