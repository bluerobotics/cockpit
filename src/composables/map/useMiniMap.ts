...
export const useMiniMap = (options: UseMiniMapOptions): UseMiniMapReturn => {
  ...
  let zoomBeforeOverview: number | undefined
  ...
  const applyOfflineView = (): void => {
    ...
    zoomBeforeOverview ??= map.value.getZoom()
  ...
  watch(
    ...
    (online) => {
      if (online) {
        if (zoomBeforeOverview !== undefined) map.value?.setZoom(zoomBeforeOverview)
        zoomBeforeOverview = undefined
      } else {
        applyOfflineView()
      }
    }
