...
import { type Go2RTCStreamInfo, WebRTCStatsEvent, WebRTCVideoStats } from '@/types/video'
...
// 10 Hz while a panel or an armed recording consumes the samples; otherwise consume the video
// store's 5 s poll so idle Standalone does not query go2rtc twice
...
const droppedRecordingsMessage = (count: number): string =>
  count === 1
    ? 'Stopped recording 1 video stream statistic that is no longer available. You can select it again once its stream is back.'
    : `Stopped recording ${count} video stream statistics that are no longer available. You can select them again once their streams are back.`
...
export const dropStaleStreamStatRecordings = (liveInternalNames: string[] | undefined): void => {
  ...
  openSnackbar({
    message: droppedRecordingsMessage(staleIds.length),
    variant: 'warning',
    duration: 10000,
    action: {
      label: 'Data-lake',
      handler: (): void => {
        logUserAction('Opened the Data-lake tool')
        goToMenuPage(SubMenuComponentName.ToolsDataLake)
      },
    },
  })
}
...
const initialize = (): void => {
  ...
  const applyGo2rtcStreamsInfo = (allInfo: Record<string, Go2RTCStreamInfo>): void => {
  ...
  const sampleGo2rtcStreams = async (): Promise<void> => {
    ...
    try {
      ...
      applyGo2rtcStreamsInfo(allInfo)
    } catch {
  ...
  const isGo2rtcFastSampling = (): boolean => go2rtcPanelConsumers > 0 || isAnyActiveRtspStreamArmed()
  ...
  const runGo2rtcSampler = async (): Promise<void> => {
    ...
    // Idle consumes the video store's 5 s poll; only keep our own chain while something needs 10 Hz
    if (!isGo2rtcFastSampling()) return
    go2rtcSamplerTimer = setTimeout(() => void runGo2rtcSampler(), go2rtcFastSampleIntervalMs)
  }

  pokeGo2rtcSampler = (): void => {
    if (!go2rtcSamplerEnabled) return
    // An in-flight tick already has the updated consumer count; its reschedule will use it
    if (go2rtcSamplerRunning) return
    if (go2rtcSamplerTimer !== null) {
  ...
  collectorScope.run(() => {
    ...
    watch(
      () => videoStore.go2rtcStreamInfo,
      (allInfo) => {
        if (!go2rtcSamplerEnabled || isGo2rtcFastSampling()) return
        applyGo2rtcStreamsInfo(allInfo)
      }
    )
    watch(
      ...
      (hasActive) => {
        ...
        if (hasActive) {
          if (isGo2rtcFastSampling()) {
            if (go2rtcSamplerTimer === null && !go2rtcSamplerRunning) {
              go2rtcSamplerTimer = setTimeout(() => void runGo2rtcSampler(), 0)
            }
          } else {
            applyGo2rtcStreamsInfo(videoStore.go2rtcStreamInfo)
          }
