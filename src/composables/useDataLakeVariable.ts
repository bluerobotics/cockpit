...
import { convertValue, readingLengthsAs } from '@/libs/units'
...
export function useDataLakeVariable(variableId: MaybeRefOrGetter<string | undefined>): {
  ...
  const converted = computed(() => {
    ...
    // A telemetry length is almost always a height, a depth or a range, so it is read in the altitude unit.
    return convertValue(value.value, rawUnit.value, readingLengthsAs(interfaceStore.displayUnitPreferences, 'altitude'))
  })
