...
export const useFenceMapInteraction = (options: UseFenceMapInteractionOptions): UseFenceMapInteractionReturn => {
  ...
  const handleFenceKeyDown = (event: KeyboardEvent): void => {
    ...
    if (event.key === 'Escape') {
      if (fenceStore.isDrawingPolygon) fenceStore.cancelDrawingPolygon()
      if (fenceStore.isDrawingCircle) fenceStore.cancelDrawingCircle()
      return
    }
    if (event.key === 'Enter') {
      if (fenceStore.isDrawingPolygon) {
        fenceStore.finishDrawingPolygon()
      } else if (fenceStore.isDrawingCircle && fenceStore.pendingCircleCenter && fenceStore.pendingCircleRadius >= 1) {
