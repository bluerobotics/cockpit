...
export const useFenceMapInteraction = (options: UseFenceMapInteractionOptions): UseFenceMapInteractionReturn => {
  ...
  // The draft is a module singleton, so an unfinished draw outlives both the
  // fence mode and the view itself, and only this composable can service one.
  // Left alone it would put a crosshair back on the mission map on the next mount.
  ...
  watch(inFenceMode, (active) => {
    if (!active) cancelDrawing()
  })

  onBeforeUnmount(cancelDrawing)
  ...
  const handleFenceKeyDown = (event: KeyboardEvent): void => {
    ...
    if (event.key === 'Escape') {
      ...
      if (fenceDraft.isDrawingCircle) {
        logUserAction('Discarded the in-progress circle fence drawing')
