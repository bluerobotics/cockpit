...
class DataLogger {
  ...
  set logInterval(value: number) {
    if (value < 1) {
      showDialog({ message: t('Minimum log interval is {min} milliseconds'), variant: 'error' })
      return
    }
  ...
  set frequency(value: number) {
    if (value > 1000 || value < 0.1) {
      showDialog({ message: t('Log frequency must be between {min} and {max} Hz'), variant: 'error' })
      return
    }
