...
import { convertValue, formatValueWithUnit, readingLengthsAs } from './units'
...
class DataLogger {
  ...
  startLogging(requesterId: string): void {
    ...
    const logRoutine = async (): Promise<void> => {
      ...
      const depth = convertValue(-vehicleStore.altitude.msl?.toJSON().value, 'm', readingLengthsAs(unitPrefs, 'depth'))
