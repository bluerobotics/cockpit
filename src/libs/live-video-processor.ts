...
export class LiveVideoProcessor {
  ...
  private async initializeOutputFile(firstChunk: Blob): Promise<void> {
    ...
    this.concatProcess = await window.electronAPI?.startLiveVideoConcat(
      firstChunk,
      this.recordingHash,
      this.fileName,
  ...
  static async processZipFile(
    ...
    try {
      ...
      const { id: processId, outputPath } = await window.electronAPI.startLiveVideoConcat(
        firstChunkBlob,
        hash,
        fileName,
      ...
      for (let i = 1; i < chunkPaths.length; i++) {
        ...
        await window.electronAPI.appendChunkToLiveVideoConcat(processId, chunkBlob, i)
      ...
      await window.electronAPI.finalizeLiveVideoConcat(processId)
