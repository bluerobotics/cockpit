...
export const updateDataLakeVariableInfo = (variable: DataLakeVariable): void => {
  ...
  const oldVariable = dataLakeVariableInfo[variable.id]
  dataLakeVariableInfo[variable.id] = variable
  ...
  // If persistValue is being turned on, save the current value
  if (variable.persistValue && !oldVariable.persistValue && dataLakeVariableData[variable.id] !== undefined) {
    savePersistentValues()
  }
  // If persistValue is being turned off, we don't need to do anything special as the value will just not be saved anymore
