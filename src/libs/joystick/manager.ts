...
export type JoystickStateEvent = {
  ...
  gamepad: Gamepad
  /**
   * Raw joystick state from Electron (SDL)
   */
  rawState?: SDLControllerState
}
...
export type GamepadsMap = Map<number, Gamepad>
...
type CallbackJoystickConnectionEventType = (event: GamepadsMap) => void
...
class JoystickManager {
  ...
  private joysticks: GamepadsMap = new Map()
    ...
    for (const gamepad of gamepads) {
      ...
      if (previousState) {
        ...
        gamepad.buttons.forEach((button, index) => {
          if (
            previousState.state.buttons[index].pressed !== button.pressed ||
            previousState.state.buttons[index].value !== button.value
          ) {
            shouldEmitStateEvent = true
          }
        })
