...
import { type MissionLoadingCallback, type Waypoint, defaultLoadingCallback, WaypointType } from '@/types/mission'
...
export abstract class ArduPilotVehicle<Modes> extends Vehicle.AbstractVehicle<Modes> {
  ...
  async fetchHomeWaypoint(): Promise<Waypoint> {
    await this.sendCommandLong(MavCmd.MAV_CMD_GET_HOME_POSITION)
    const startTime = new Date().getTime()
    let homePosition: Message.HomePosition | undefined = undefined
    while (!homePosition && new Date().getTime() - startTime < 5000) {
      await sleep(100)
      homePosition = this._messages.get('HOME_POSITION') as Message.HomePosition
    }
    if (!homePosition) {
