...
const fetchOpenCellIdCellsQueued = async (bbox: OpenCellIdBboxRequest): Promise<NearbyOpenCellIdCell[]> => {
  ...
  const runWithBackoff = async (): Promise<NearbyOpenCellIdCell[]> => {
    ...
    for (let attempt = 0; attempt <= OPEN_CELL_ID_TOO_MANY_REQUESTS_BACKOFF_MS.length; attempt++) {
      try {
        ...
        const rateLimited = isTooManyRequestsError(error)
        if (rateLimited && attempt < OPEN_CELL_ID_TOO_MANY_REQUESTS_BACKOFF_MS.length) {
          await sleep(OPEN_CELL_ID_TOO_MANY_REQUESTS_BACKOFF_MS[attempt])
          continue
        }
        if (rateLimited) openCellIdRateLimitUntilMs = Date.now() + OPEN_CELL_ID_RATE_LIMIT_COOLDOWN_MS
        throw error
      }
    }
    throw new Error('OpenCellID: request failed after exhausting the retry backoff.')
  }

  const request = openCellIdQueue
    .catch(() => undefined)
    .then(runWithBackoff)
    .finally(() => {
  ...
  // The serializer has to survive a failed request, so the queue tail swallows the rejection while
  // the caller still receives it.
  openCellIdQueue = request.then(
    () => undefined,
    () => undefined
  )
